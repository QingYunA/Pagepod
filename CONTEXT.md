...
- **GuestTransient (Token-Protected Guest Link)**: The ephemeral access state assigned to unauthenticated single-HTML uploads. Distinct from legacy unlisted URLs, it pairs an unindexed route (`noindex`) with a client-side cryptographic `ClaimToken`, granting the uploader transient management authority while preventing public feed discovery until explicitly claimed or promoted.
- **Custom Subdomain (`customSubdomain`)**: A Pro-tier project routing capability dynamically mapping `https://${slug}.pagepod.dev` (or a dedicated custom host) directly to the isolated runner `/p/${slug}`, bypassing central portal framing while preserving sandbox CSP boundaries.
- **White-Label Mode (`isWhiteLabel`)**: A Pro-tier project presentation toggle that suppresses the viral "Hosted on Pagepod" floating badge on the project runner (`/p/[slug]`), enabling unbranded commercial and client-facing demos.
...
- **ProjectSeoProfile**: A structured domain profile binding a public project to its programmatic SEO assets: primary target keyword, secondary long-tail intents, rich canonical description, key use cases, structured FAQ pairs (`question`, `answer`), and Schema.org rich snippets (`FAQPage`, `SoftwareApplication`, `BreadcrumbList`).
- **Organic Ingress Asset**: A hosted public project (`/p/[slug]`) architected as an autonomous search landing entry point targeting high-volume, low-KD queries, pairing an instant sandbox runner on the primary viewport with server-rendered semantic depth (RSC) on the secondary viewport.
- **Strict Unilingual Invariant**: The search indexing requirement that a project's SEO metadata, structured text, FAQs, and JSON-LD schema must strictly match the project's native interface language (`zh` or `en`), prohibiting mixed-language machine translation.
- **Curated Open-Source Provenance**: The compliance and quality invariant requiring all scaled showcase applications to originate from authentic, vetted open-source repositories with verifiable permissive licenses (`MIT`, `Apache-2.0`, `BSD`, `CC0`) and genuine author attribution, strictly rejecting synthetic AI-generated filler.
...
15. **Pro Project Customization Invariant**: Custom subdomains and white-label badges are evaluated per project record. Non-pro users attempting to toggle `isWhiteLabel` or register custom subdomains are blocked at the domain service layer.
16. **Soft-Gated Creator Credential Warning**: Authenticated project creation in `/workspace/upload` scans code on submission for sensitive keys; detected matches present a soft-gating modal offering one-click downgrade to `private` or explicit acknowledged consent before public persistence.
17. **Organic Ingress & Unilingual SEO Invariant**: Public project runner pages (`/p/[slug]`) must render rich server-side semantic depth (what it is, use cases, FAQ accordion, Schema.org `SoftwareApplication` + `FAQPage` + `BreadcrumbList`) strictly matching the project's native language (`zh` or `en`) without synthetic mixed-language translation artifacts.
18. **Curated Open-Source Provenance Invariant**: Scaled showcase applications must originate from verified open-source repositories with permissive licensing (`MIT`, `Apache-2.0`, `BSD`, `CC0`), verifiable author/repo attribution, and support for standard external CDNs and multi-file assets per ADR 0002.
